...
## 点击状态栏返回顶部是 Kuikly 提供的功能吗？

**不是**。点击状态栏返回顶部是**国产 Android 厂商（如 OPPO ColorOS、小米 MIUI 等）的系统级差异化功能**，国际原生 Android 系统（如 Google Pixel、Android One 等）并不支持此特性。

### 常见问题

**为什么在我的手机上可以点击状态栏返回顶部，但在其他设备上不行？**

这是因为：
1. **Android定制系统**：ColorOS、MIUI、HarmonyOS 等ROM 添加了点击状态栏滚动到顶部的功能
2. **原生系统不支持**：Google Pixel、Nexus 等国际原生 Android 设备无此功能
3. **触发机制差异**：部分系统仅在特定条件下触发（如列表内容足够长、当前页面在顶部有可见滚动容器等）

### 如何在 Kuikly 中统一处理

Kuikly Dsl: [`scrollToTop`](../API/components/scroller.md#scrolltotop) 事件：
Compose Dsl : [可滚动组件扩展](../Compose/core-components.md#可滚动组件扩展) 文档。
