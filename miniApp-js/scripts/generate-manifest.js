...
const MANIFEST_JSON_PATH = path.join(BUNDLES_DIR, 'manifest.json');
const MANIFEST_JS_PATH = path.join(BUNDLES_DIR, 'manifest.js');
...
function generateManifest() {
  ...
  // 生成 .json 文件（用于查看）
  fs.writeFileSync(MANIFEST_JSON_PATH, JSON.stringify(manifest, null, 2), 'utf8');

  // 生成 .js 文件（用于小程序 require）
  const jsContent = `// Auto-generated manifest file
...
`;
  ...
  console.log('[Manifest Generator] 📁 JSON 路径:', MANIFEST_JSON_PATH);
  console.log('[Manifest Generator] 📁 JS 路径:', MANIFEST_JS_PATH);
