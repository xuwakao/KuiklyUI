...
class LogTestModule : Module() {
    ...
    fun test() : String {
        ...
        // 当前测试Module返回值为 Map ( Module 在C++层）
        ...
        return buildString {
            ...
            val deep = nested?.get("deep") as? Map<String, Any?>
            val deepLevel3Match = deep?.get("level3") == "深层嵌套"
            
            appendLine("【nested.key1】: $nestedKey1Match (${nested?.get("key1")})")
            appendLine("【nested.key2】: $nestedKey2Match (${nested?.get("key2")})")
            appendLine("【nested.deep.level3】: $deepLevel3Match (${deep?.get("level3")})")
            ...
            val allMatch = stringMatch && intMatch && floatMatch && negativeMatch &&
                ...
                nestedKey1Match && nestedKey2Match && deepLevel3Match &&
