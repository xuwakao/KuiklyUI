# render-sync-send-event-policy Specification

## Purpose
提供跨端（iOS / Android / HarmonyOS）统一的宿主层同步事件发送策略机制。宿主/Delegator 层决定哪些页面事件需要同步派发到 Kotlin 侧，渲染层负责执行。默认将 `onBackPressed` 和 `rootViewSizeDidChanged` 设为同步事件，解决页面旋转/尺寸变化时因异步派发导致的白屏问题。
## Requirements
...
- **WHEN** iOS delegator decides an event must be synchronous through `syncSendEvent:`
...
- **WHEN** HarmonyOS host controller decides an event must be synchronous through `syncSendEvent(event)`
