...
| 场景 | 原因 | 状态 |
|------|------|------|
| **首次 Composition** | `invalidationMap` 为空，`onBeginScopeComposition` 不被调用，`activeScopeStack` 始终空 | 预期行为 |
| **参数驱动重组，且祖先链上没有 scope 在 `invalidationMap` 中** | 比如整个调用链都是参数传递驱动，没有 State 直接失效的 scope | 预期行为 |
| **~~框架组件被 filter 截断~~** | ~~祖先 scope 存在但被 `shouldFilterComposable` 过滤，子节点在 `traceEventEnd` 时丢失 scope~~ | **已修复**（scopeKeySnapshot） |
| **LazyColumn subcomposition** | sub-composition 的 `invalidationMap` 为空，scope observer 未注册 | 架构限制 |
...
- **`scopeKey` 不一致性**：State 失效驱动时有祖先 scope key，参数驱动时为 null。框架组件 filter 截断导致的 scope 丢失已通过 `scopeKeySnapshot` 修复（在 `traceEventStart` 时捕获 scope 快照）；LazyColumn sub-composition 的 scope 缺失仍为 Compose Runtime 架构限制
